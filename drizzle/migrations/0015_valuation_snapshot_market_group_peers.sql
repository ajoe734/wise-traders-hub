-- 0015: 同業母體改為「市場族群優先」
-- 母體優先序：group（market_groups 重疊）→ fine（細分產業）→ broad（產業大類）→ legacy
-- 每層需至少 3 家同業有當日有效 PER 或 PBR 才成立，否則往下退。
...
AS $function$
  ...
  ind AS (
    SELECT
      ...
      COALESCE(array_length((SELECT market_groups FROM me), 1), 0) > 0 AS has_group,
      (SELECT market_groups[1] FROM me) AS group_name,
  ...
  group_syms AS (
    ...
    WHERE ind.has_group
      AND m.market_groups && (SELECT market_groups FROM me)
      AND m.symbol <> sym.s
  ),
  fine_syms AS (
    SELECT m.symbol, m.market
...
AS $function$
  ...
  snaps AS (
    SELECT public.valuation_snapshot(n.symbol) AS snap
    FROM norm n
  )
  SELECT COALESCE(jsonb_agg(elem ORDER BY elem->>'symbol'), '[]'::jsonb)
  FROM (
    SELECT jsonb_build_object(
      'symbol', snap->>'symbol',
      'asOf', snap->>'asOf',
      ...
      'marketGroups', COALESCE(snap->'marketGroups', '[]'::jsonb),
      'peerScope', snap->>'peerScope',
      'peerCount', snap->'peerCount',
      'peerIndustry', snap->>'peerIndustry',
      'peers', COALESCE((
        SELECT jsonb_agg(jsonb_build_object(
          'symbol', p->>'symbol',
          'pe', p->'pe',
        ...
        FROM jsonb_array_elements(snap->'peers') AS p
      ), '[]'::jsonb)
    ) AS elem
    FROM snaps
...
GRANT EXECUTE ON FUNCTION public.valuation_snapshot(text) TO anon, authenticated, service_role;
