-- 0017: 同業母體五段優先序，主產業比對優先，收斂同業家數
--   group    ── 最精準的市場族群（成員 3–20 家，取成員數最少者）
--   fine     ── 主產業相同（industries[1] 對 industries[1]）
--   fineWide ── 細分產業任一重疊（舊行為，保底）
--   broad    ── 產業大類
--   legacy   ── tw_industry_peers
...
AS $function$
  ...
  group_syms AS (
    ...
    WHERE ind.group_name IS NOT NULL
      AND m.market_groups @> ARRAY[ind.group_name]
      AND m.symbol <> sym.s
  ),
  fine_syms AS (
    SELECT m.symbol FROM public.stock_industry_map m CROSS JOIN ind CROSS JOIN sym
    WHERE ind.fine
      AND m.industries[1] = (SELECT industries[1] FROM me)
      AND m.symbol <> sym.s
  ...
  SELECT jsonb_build_object(
    ...
    'trend', COALESCE((
      SELECT jsonb_agg(jsonb_build_object('date', to_char(trade_date, 'YYYY-MM-DD'), 'pe', per, 'pb', pbr, 'dividendYield', dividend_yield) ORDER BY trade_date)
      FROM trend_raw), '[]'::jsonb),
    'peers', COALESCE((
