-- 0016: 市場族群母體改取「最精準的那一個族群」
-- 一檔最多 3 個族群，直接用陣列重疊會把「台灣50成分股」「輝達供應鏈概念」這類大族群一起吃進來（100+ 家）。
-- 規則：在該檔的族群中挑成員數最少者，且成員數需 3–20 家（含自己）才採用；否則退回細分產業。
...
AS $function$
  ...
  group_pick AS (
    SELECT g.name AS group_name, cnt.c AS members
