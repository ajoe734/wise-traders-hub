...
async function countGridColumns(
  ...
  test('清除 flag 後，demo intro modal 會自動彈出（mobile auto-open regression guard）', async ({ page, browserName }, testInfo) => {
    // 攔截 mp4：headless 缺 H.264 codec 時避免 page crash（webkit 尤甚）
    ...
    await page.addInitScript(() => {
      try {
        ...
        // 避免 <video autoplay> 進入 media pipeline 造成 headless crash
    ...
    // 4) 手機專屬：影片應套用 9/16 直式比例（isMobile 分支）
    const aspect = await page
      .locator('[data-testid="holdings-intro-modal"] video')
      .evaluate((el) => (el as HTMLElement).style.aspectRatio);
    expect(
      aspect.replace(/\s+/g, ''),
      `[${testInfo.project.name}] mobile viewport (${testInfo.project.use?.viewport?.width}px) 應套 9/16 直式影片`,
    ).toBe('9/16');
