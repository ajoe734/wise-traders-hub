...
> **維護模式宣告**  
> 前身為 `docs/architecture/shell-event-bus-tdd.md`；原始 TDD 階段已完成，本文件進入維護模式。  
> M3 `events:refresh` 的後續真正 re-fetch 實作詳見 [`docs/architecture/events-refresh-tdd.md`](./events-refresh-tdd.md)。

> **使用約定**
> 1. 每次動工前先 `code--view docs/architecture/shell-event-bus.md`。
> 2. 契約變動（新增事件、改 payload）**先改本 doc 再改 code**。
> 3. 每完成一個 step，立刻更新第 7 章「執行日誌」的 checkbox 與測試輸出摘要。
...
| `docs/architecture/holdings-modules.md` | TODO 區塊加 `→ 詳見 shell-event-bus.md`；完成後移入「已完成」。 |
...
- [x] Shell event bus — **本 PR 落實**。

實作後把第 3 條 checkbox 打勾，並在 `holdings-modules.md` 對應段落加上「詳見 shell-event-bus.md」。
