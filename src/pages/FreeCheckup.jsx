...
const PARSE_PROMPT = `你是台股券商庫存/持倉截圖與成交回報截圖的解析器。解析截圖中的每一筆交易或持倉，以JSON格式輸出，不輸出其他文字：
{"trades":[{"action":"買進或賣出或庫存","code":"代碼","name":"名稱","qty":股數,"available_qty":可用股數或null,"price":成交均價或成本價,"market_price":市價或現價,"cost":成本金額或null,"value":市值或null,"pnl":預估損益或null,"pnl_percent":報酬率百分比數字或null,"net_settlement":預估淨收付或null,"amount":成交金額或null}],"targetPriceUpdates":[{"code":"代碼","firm":"券商名稱","target":目標價數字,"date":"日期"}],"note":"有疑問時說明"}

【截圖可能出現的欄位對照】
截圖中可能出現以下欄位（以富貴角10號等券商 App 為例）：
- 商品（含股名+代號）→ name + code
- 股數 → qty
- 可用股數 → available_qty
- 市價/現價 → market_price（即時或收盤價格）
- 市值 → value（= market_price × qty）
- 成本價/成交均價 → price（每股平均成本）
- 成本 → cost（= price × qty，總成本金額）
- 預估損益 → pnl
- 報酬率(%) → pnl_percent（純數字，例如 -3.28 表示 -3.28%）
- 預估淨收付 → net_settlement
...
2. market_price（市價/現價）同樣必須精確保留完整小數位數，不可四捨五入。若截圖中無此欄位則填 null。
3. qty（股數）必須精確，例如 3000股 就是 3000，2000股 就是 2000，2股 就是 2。
4. 權證名稱通常包含「購」「售」「牛」「熊」等字，其價格可能很小（如 0.61、1.55），務必精確辨識。
5. 如果截圖是庫存/持倉畫面（非成交回報），action 請填「庫存」。
6. pnl_percent 只填數字，例如截圖顯示 -3.28% 就填 -3.28。
7. 所有金額與價格欄位都必須從截圖中原樣讀取，不要自行計算或推導。
