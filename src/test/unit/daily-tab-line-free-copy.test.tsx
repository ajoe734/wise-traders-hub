 ...
 * 驗證 4 種狀態的文案與 Asia/Taipei 日期一致性。
 */
...
describe('DailyTab — LINE 註冊禮文案', () => {
  it('line_free + remain=1 → 「還可使用 1 次」', () => {
    const q = { tier: 'line_free', period: 'lifetime', limit: 1, used: 0, remaining: 1, resets_at: 'infinity', last_used_at: null };
    const { container } = renderDaily(q, 'line_free', false);
  ...
  it('line_free + remain=0 + 有 last_used_at → 顯示 Asia/Taipei 使用日', () => {
    const q = { tier: 'line_free', period: 'lifetime', limit: 1, used: 1, remaining: 0, resets_at: 'infinity', last_used_at: '2026-06-03T05:30:00Z' };
    const { container } = renderDaily(q, 'line_free', true);
    const t = text(container);
  ...
  it('line_free + remain=0 + null last_used_at → fallback「使用日 尚未紀錄」', () => {
    const q = { tier: 'line_free', period: 'lifetime', limit: 1, used: 1, remaining: 0, resets_at: 'infinity', last_used_at: null };
    const { container } = renderDaily(q, 'line_free', true);
    const t = text(container);
  ...
  it('跨日：UTC 16:30 → 隔日台北 00:30', () => {
    const q = { tier: 'line_free', period: 'lifetime', limit: 1, used: 1, remaining: 0, resets_at: 'infinity', last_used_at: '2026-06-03T16:30:00Z' };
    const { container } = renderDaily(q, 'line_free', true);
