  ...
  return (
    <div
      ...
      style={{
        ...
        fontSize: 11, color: C?.textMute, letterSpacing: '0.04em',
      }}
    >
      <span>示範資料 · 尚未登入</span>
      <span style={{ display: 'inline-flex', gap: 14 }}>
        <button
          ...
          style={{ background: 'transparent', border: 'none', cursor: 'pointer', color: C?.text, fontSize: 11 }}
        >LINE 登入 →</button>
        <button
          type="button" onClick={onStartEmail}
          style={{ background: 'transparent', border: 'none', cursor: 'pointer', color: C?.textSec, fontSize: 11 }}
        >Email 登入</button>
      </span>
    </div>
  );
