...
select 'TRSHAPE|'||status||'|'||coalesce(market,'-')||'|'||coalesce(currency,'-')||'|'||coalesce(quantity_unit,'-')||'|'||is_combo||'|'||count(*) from trade_records group by 2,3,4,5,6 order by 1;
...
select 'APP|'||tg_op||'|'||count(*) from signal_trade_applications group by 2 order by 1;
...
select 'FXGAP|'||coalesce(market,'-')||'|'||coalesce(currency,'-')||'|'||count(*) from trade_records where currency is null or market is null group by 2,3;
