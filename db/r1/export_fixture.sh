#!/usr/bin/env bash
# R1-B: anonymized fixture export from production (READ-ONLY). PII hashed or nulled.
set -e
OUT=${1:-/tmp/r1fixture}
mkdir -p "$OUT"
A="md5(('r1salt'||coalesce(user_id::text,'nil')))::uuid"

psql -q -c "\copy (select id, $A as user_id, 'expert-'||left(id::text,8) as slug,
...
psql -q -c "\copy (select id, expert_id, null::uuid plan_id, instrument, action, price_hint,
 '(redacted)' reason_summary, null::text reason_detail, null::text risk_notes,
 case when learning_points is not null then '(redacted)' else null end as learning_points,
...
psql -q -c "\copy (select $A as user_id, symbol, name, current_price, pnl, pnl_percent, updated_at, signal_id, entry_price from user_performances) to '$OUT/user_performances.csv' csv header"
# market data (non-PII), scoped to instruments referenced by the economic fixture
psql -q -c "\copy (select * from daily_price_snapshots where trade_date > current_date - 400) to '$OUT/daily_price_snapshots.csv' csv header"
psql -q -c "\copy (select * from fx_rates) to '$OUT/fx_rates.csv' csv header"
psql -q -c "\copy (select * from tw_market_holidays) to '$OUT/tw_market_holidays.csv' csv header"
wc -l "$OUT"/*.csv
