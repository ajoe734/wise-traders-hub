-- =====================================================================
-- Stage 3B — S3B-A：BSR ingest suppression gate（先關入口）
--
-- 背景：FinMind 判定 provider_unsupported_plan（Stage 2：HTTP 400 "Your level is
-- register."）。gate 目前為 legacy_config_missing（blocked=true）。本 migration 只做
-- 「入口抑制」：新增 private_bsr.ingest_allowed()，並在七支 DB producer/recovery
-- 函式最前面加 fail-closed early-return。**不動 config（仍 v7）、不動 queue/backlog、
-- 不動任何 ACL、不動 cron、不部署 edge。** 1 個 legacy edge call site（tw-chips-detail）
-- 本階段不動。
--
-- fail-closed 契約（與 supabase/tests/bsr_gate_ingest_allowed_test.sql 的 9 case 一致）：
--   gate row 缺席 / legacy（無 admission_blocked 鍵）/ config 非 object /
--   admission_blocked 型別不符 / admission_blocked=true  → ingest_allowed() = false
--   只有 canonical `admission_blocked` 恰為 JSON false     → true
-- 沒有任何 default-allow 路徑。
--
-- HARD STOP 前置（本次套用前實測）：
--   has_schema_privilege('anon'|'authenticated'|'service_role','public','CREATE') = f/f/f
--   public nspacl 僅 pg_database_owner=UC，其餘 =U
--   → search_path = pg_catalog, public 對七支 SECURITY DEFINER 安全。
--
-- 七支函式一律 CREATE OR REPLACE：RETURNS / 參數 / owner / prosecdef 完全不變，
-- ACL 由 CREATE OR REPLACE 保留不變；只有 proconfig 由 search_path=public 改為
-- search_path=pg_catalog,public，以及函式最前面多一段 early-return guard。
-- 早退回傳型別相容：jsonb 六支保留原必備鍵（0/false）再加 "skipped":"bsr_provider_unsupported"；
-- trigger 版直接 RETURN NEW；enqueue_bsr_backfill 回 integer 0。
--
-- 恢復 runbook（本輪不執行，僅記錄）：未來 bsr_unblock_after_probe 成功時，同交易
--   UPDATE public.tw_bsr_sync_queue SET status='pending', next_run_at=now()
--    WHERE status='skipped' AND id = ANY(<S3B-E ids>);
--
-- ---------------------------------------------------------------------
-- SEMANTIC ROLLBACK：
--   1) 以 db/r1/c/S3BA/preimage_7fns.sql 內七支 exact CREATE OR REPLACE 覆蓋回去
--      （該檔為套用前 pg_get_functiondef 逐字輸出，含原 search_path=public）。
--   2) DROP FUNCTION IF EXISTS private_bsr.ingest_allowed();
--   （不觸及任何資料列、ACL、config、queue。）
-- =====================================================================
...
-- ---------------------------------------------------------------- helper
-- private_bsr 已於 Stage 1 建立且對 PUBLIC 無任何權限；此處不再 GRANT 任何角色。
-- SECURITY DEFINER 理由：呼叫端（七支 DEFINER 函式以 owner=postgres 執行）需要
-- 以固定身分讀 gate 狀態；STABLE + 固定 search_path 避免被呼叫端 search_path 置換。
-- search_path 不含 public：唯一依賴 private_bsr.gate_state() 自身已 schema-qualify
-- public.tw_bsr_sync_config。
...
-- ---------------------------------------------------------------- guard 1/7: public.enqueue_chips_prefetch_gaps(integer,integer)
...
BEGIN
  IF NOT private_bsr.ingest_allowed() THEN
    RETURN jsonb_build_object(
      ...
      'stocks_with_gaps', 0,
      'inserted', 0,
      'recovery', jsonb_build_object('running_reset', 0, 'retry_requeued', 0,
                                     'skipped', 'bsr_provider_unsupported'),
      'backpressure', NULL,
      'quota_recovery', jsonb_build_object('tokens_issued', 0, 'reconciled', 0,
                                           'recovered', 0,
                                           'skipped', 'bsr_provider_unsupported'));
  END IF;
...
-- ---------------------------------------------------------------- guard 2/7: public.enqueue_all_active_tw_holdings_bsr(integer)
...
-- ---------------------------------------------------------------- guard 3/7: public.enqueue_bsr_first_fetch_on_trade()
...
-- ---------------------------------------------------------------- guard 4/7: public.ensure_bsr_queued(text)
...
-- ---------------------------------------------------------------- guard 5/7: public.enqueue_bsr_backfill(text,integer)
...
-- ---------------------------------------------------------------- guard 6/7: public.recover_stale_bsr_queue_jobs(integer,integer)
...
-- ---------------------------------------------------------------- guard 7/7: public.recover_quota_failed_bsr_jobs(integer)
...
BEGIN
  IF NOT private_bsr.ingest_allowed() THEN
    RETURN jsonb_build_object(
      ...
      'recovered', 0,
      'remaining', NULL,
      'budget_reason', 'bsr_provider_unsupported',
      'counting_mode', 'exact',
      'tokened_job_ids', '[]'::jsonb,
      'reconciled_job_ids', '[]'::jsonb);
  END IF;
