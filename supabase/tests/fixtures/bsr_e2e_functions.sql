...
-- 載入順序無關：SQL-language 函式互相引用時關閉 body 驗證（僅影響載入期，不改函式定義）
...
$function$
;
CREATE OR REPLACE FUNCTION public.detect_institutional_gap_jobs(_target_date date DEFAULT CURRENT_DATE, _lookback_days integer DEFAULT 60, _max_jobs integer DEFAULT 5000)
 RETURNS TABLE(stock_id text, start_date date, end_date date, gap_count integer)
 LANGUAGE sql
 STABLE SECURITY DEFINER
 SET search_path TO 'public'
