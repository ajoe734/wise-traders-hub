...
// Taipei 日期時間（YYYY/MM/DD HH:mm）。Intl 保證與前台 formatTaipeiYMDHM 一致。
...
): 'lot' | 'share' | 'contract' | 'other' | 'missing' {
  const raw = (unit ?? '').trim();
...
export function toShares(qty: number, unit: ReturnType<typeof normalizeQuantityUnit>): number {
...
export function detectExportRisks(
  ...
  for (const [key, bucket] of byKey) {
    ...
    const missingUnitRows: JournalRowExport[] = [];
    ...
    if (missingUnitRows.length > 0) {
      const firstMissing = missingUnitRows[0];
      const asset = firstMissing.experts?.asset_class ?? '';
...
/** zip 內檔名去重：slug 撞名時追加 expert_id，避免後者覆蓋前者。 */
export function uniqueMentorFilename(
