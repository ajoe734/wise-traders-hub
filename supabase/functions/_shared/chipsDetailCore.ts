// _shared/chipsDetailCore.ts
// 抽出 tw-chips-detail / tw-chips-batch 共用的 payload 建構邏輯。
// 本檔不處理 HTTP、快取、request coalescing —— 純粹組裝單一 stock_id 的完整籌碼摘要。
// deno-lint-ignore-file no-explicit-any

import { countRowsByDate, pickCompleteFallbackDate, DONE_BROKER_THRESHOLD, LOW_QUALITY_BROKER_THRESHOLD } from "./bsrRollup.ts";
import { expectedLatestBsrDate, weekdayDiff } from "./tradingDate.ts";
import { resolveAllWindows } from "./seriesReadiness.ts";
...
export async function buildChipsPayload(supa: any, stockId: string): Promise<any> {
  // ==== 三大法人 1/5/20/60 日 ====
