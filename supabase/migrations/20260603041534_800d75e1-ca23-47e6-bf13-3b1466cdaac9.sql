-- Harden check_checkup_quota against last_used_at anomalies:
-- 1) Wrap usage lookup in EXCEPTION block so a broken row never blocks the whole RPC
-- 2) Ensure NULL is explicit (already in place but reaffirmed)
-- 3) Guarantee last_used_at is always present in the returned jsonb (never missing key)
...
BEGIN
  ...
  RETURN jsonb_build_object(
    ...
    'last_used_at', v_last_used_at  -- 永遠存在這個 key（值可能為 NULL）
  );
END;
$$;
