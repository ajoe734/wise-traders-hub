-- M5 根因修復：M4 遷移覆寫 converge_bsr_windows 時遺失 M1 決定性排序。
-- 這裡合併兩者：threshold=1（M4）+ ORDER BY valid_days ASC, last_valid ASC NULLS FIRST（M1）。
-- 相關合約鎖在 src/test/integration/bsr-window-fulfillment.test.ts。
               ...
               (SELECT COUNT(*)::int
                  FROM (
                    SELECT trade_date
                     ...
                     GROUP BY trade_date
                    HAVING COUNT(DISTINCT broker_id) >= 1  -- M4 門檻
                  ) x
               ), 0) AS valid_days,
