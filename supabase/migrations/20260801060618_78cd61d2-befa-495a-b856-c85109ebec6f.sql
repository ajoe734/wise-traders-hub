...
BEGIN
  -- 效能：覆蓋率改用一次 GROUP BY 聚合（原本是每檔股票兩個相關子查詢，
  -- 在 tw_bsr_daily 長大後必然 statement timeout）。
  ...
  cov AS (
    SELECT stock_id,
      ...
      FROM (SELECT stock_id, trade_date FROM public.tw_bsr_daily
             GROUP BY stock_id, trade_date) g
     GROUP BY stock_id
  )
  SELECT s.sid,
         COALESCE(cov.valid_days, 0) AS valid_days,
         cov.last_valid
    FROM all_stocks s
    LEFT JOIN cov ON cov.stock_id = s.sid;
