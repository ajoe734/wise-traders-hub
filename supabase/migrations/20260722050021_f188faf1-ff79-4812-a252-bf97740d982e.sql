...
BEGIN
  ...
  IF NEW.status IN ('published', 'pending') THEN
    ...
    -- trade_records.quantity stores actual base units used for cost/market-value math.
    -- 台股「張」必須轉成實際股數；股/口/顆維持原始數量。
    v_trade_qty := CASE
...
COMMENT ON FUNCTION public.handle_signal_trade() IS 'Stores trade_records.quantity in base units: 台股張 -> 股數, 股/口/顆 keep raw quantity; derives market/currency from expert.asset_class.';