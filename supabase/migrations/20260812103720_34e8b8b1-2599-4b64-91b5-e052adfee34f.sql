-- ============================================================
-- Build 1b: recovery liveness / backlog metric / degrade self-heal
-- ============================================================

-- 1) Read-only backlog metrics (single source of truth)
...
WITH tpe AS (
  ...
  SELECT f.stock_id, count(DISTINCT f.trade_date) AS have5
    FROM public.tw_chip_fact f, tpe
   WHERE f.stock_id IN (SELECT stock_id FROM cohort)
     AND f.trade_date >= tpe.today_tw - 10
   GROUP BY f.stock_id
), classified AS (
  SELECT c.*,
...
-- 2) Recovery budget: layered gates by real degrade reason + pool reserve
...
-- 3) Recovery executor: atomic classify -> reconcile -> token -> audit
...
BEGIN
  ...
  IF v_lock IS NOT TRUE THEN
    ...
    RETURN jsonb_build_object('invocation_id', v_invocation, 'tokens_issued', 0, 'reconciled', 0,
                              'budget_reason', 'lock_contended', 'counting_mode', 'exact');
  END IF;
  ...
  -- 3a) terminal reconcile (no API): failed rows whose stock/date already has fact
  WITH pick AS (
    SELECT q.id
      FROM public.tw_bsr_sync_queue q
     WHERE q.status = 'failed'
  ...
  -- 3b) recovery token (bounded candidates, pool-aware, cap 1)
  IF v_cap > 0 THEN
    WITH tpe AS (SELECT v_today_tw AS today_tw),
    cand AS (
    ...
    ready5 AS (
      SELECT f.stock_id, count(DISTINCT f.trade_date) AS have5
        FROM public.tw_chip_fact f, tpe
       WHERE f.stock_id IN (SELECT stock_id FROM cand)
         AND f.trade_date >= tpe.today_tw - 10
       GROUP BY f.stock_id
    ),
    actionable AS (
    ...
    SELECT count(*) INTO v_inspected
      FROM public.tw_bsr_sync_queue q
     WHERE q.status IN ('failed')
       AND (q.last_error LIKE 'finmind_admission_%' OR q.last_error = 'quota_deferred');
    v_inspected := LEAST(v_inspected, CANDIDATE_LIMIT);
  END IF;
  ...
  -- legacy keys kept for backwards compatibility with existing callers/logs
