...
-- ============================================================
-- 1. Unique index on provider_tx_id to prevent race-condition duplicates
-- ============================================================
CREATE UNIQUE INDEX IF NOT EXISTS idx_payment_tx_provider_tx_id_unique
ON public.payment_transactions (provider_tx_id)
WHERE provider_tx_id IS NOT NULL;

-- ============================================================
-- 2. Fix handle_signal_trade: exit → 'closed' instead of 'stopped'
-- ============================================================
...
BEGIN
  ...
  IF NEW.status IN ('published', 'pending') THEN
    ...
    ELSIF NEW.action = 'exit' THEN
      -- FIX: Use 'closed' instead of 'stopped' for consistency
      UPDATE public.trade_records
      SET exit_price = NEW.price_hint,
          exit_date = COALESCE(NEW.published_at, NOW()),
          pnl_percent = CASE
